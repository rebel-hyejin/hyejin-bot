...
- **Co-Authored-By trailer 금지** — rebellions-sw 레포는 checkpatch warning을 유발하므로 commit message에 `Co-Authored-By:` 트레일러 포함 금지. PR에 들어 있으면 `[D7]` MAJOR.
- **Sign-off (`Signed-off-by:`) 필수** — `git commit -s` 항상. 누락은 `[D8]` MAJOR.
- **Convention 문서 선행 확인** — `inv/`, `.github/workflows/`, `test/system/` 변경 시 해당 `docs/conventions/*.md`를 원본 그대로 직접 참조했는지 확인. 에이전트 요약을 인용한 흔적이 있으면 `[D9]` MAJOR. 기존 코드의 위반을 그대로 옮긴 것은 컨벤션에 맞게 수정할 기회로 — 그렇게 안 했으면 동일하게 `[D9]`.
- **PR base 명시** — default branch가 `dev`인 레포에서 release line(`release/v3.3` 등) 타겟 PR은 `gh pr create --base release/x.x` 명시 필수. base/head 불일치 PR은 `[D10]` CRITICAL.
- **Release backport 의존 헬퍼 검증** — `dev → release/v3.x` cherry-pick 충돌 후 의존 헬퍼를 함께 backport한 경우, 그 헬퍼가 **target deploy host에서 실제로 필요한지** 실측한 증거가 PR description 또는 commit 메시지에 있어야 한다. 없으면 `[D11]` MAJOR. 우회 가능한데 명목상 backport한 케이스는 같은 등급.

## Robot Framework / test 규칙

- **Then 절 PASS-only** — `The test result SHOULD BE PASS` 류의 결과 검증 키워드는 PASS만 pass 처리. gtest 등 외부 러너가 SKIPPED 반환 시 `builtin.skip()`이 아니라 `builtin.fail()` 호출 + "check FW for the underlying cause" 메시지 권장. 위반은 `[T8]` MAJOR.
- **Given/Setup 환경 가드만 Skip 허용** — 디바이스 수 부족 등 *런타임 환경 불충족*은 RF 컨벤션대로 `Skip`/`Skip If` 유지. Then 절 결과 해석에서 SKIP을 통과시키는 것과 혼동 금지.
- **테스트도 같이 갱신** — SKIP→FAIL 변경 시 기존 `test_skip_when_*` 케이스를 `test_fail_when_*`로 의도 명시. 누락은 `[T9]` MINOR.
- **time.sleep mock 필수** — unit test에서 `time.sleep` 호출은 반드시 mock. 누락은 `[T10]` MAJOR (CI 시간 budget 직격).
- **Test mock 정확성** — mock이 실제 프레임워크 동작과 일치해야 함 (예: Fabric `warn=False` 기본값, non-zero exit → `UnexpectedExit` raise). `warn=True` 사용 시 `Result.ok` 체크 누락은 `[T11]` MAJOR.
...
| **Backport PR (`release/*` 타겟)** | base/head 검증 먼저: `gh pr view <n> --json baseRefName,headRefName`. base가 release line이 아니면 `[D3]` CRITICAL로 시작. 의존 헬퍼 backport 여부는 PR description의 실측 증거 확인. |
...
4. **카탈로그로 매칭.** [references/anti-patterns.md](references/anti-patterns.md) 의 카테고리(Clean Code Naming/Functions/General/Comments · Pipeline · Test Determinism · Secret/Runner · Observability · IaC · NPU Lab · Drift · **Repo-Specific (ssw-bundle/rebellions-sw)** · **Robot Framework**) 순서로 훑기. 해당 룰 ID(`[N7]`, `[F1]`, `[G35]`, `[P1]`, `[O1]`, `[T1]`, `[D7]`, `[T2]` …)를 인용.
...
|---|---|---|---|
| `[D7]` | Drift / commit hygiene | Co-Authored-By trailer 금지 (rebellions-sw checkpatch warning) | MAJOR |
| `[D8]` | Drift / commit hygiene | Signed-off-by trailer 누락 (`git commit -s` 항상) | MAJOR |
| `[D9]` | Drift / convention | 컨벤션 원본 문서 미참조 (에이전트 요약만 인용), 기존 코드의 위반을 그대로 복사 | MAJOR |
| `[D10]` | Drift / PR meta | release line PR의 `--base` 누락 (default branch가 dev라 자동 dev로 설정됨) | CRITICAL |
| `[D11]` | Drift / backport | 의존 헬퍼 backport 시 target deploy host 실측 증거 부재 | MAJOR |
| `[T8]` | Test determinism / RF | Robot Then 절에서 SKIP을 builtin.skip()으로 pass 처리 | MAJOR |
| `[T9]` | Test determinism / RF | SKIP→FAIL 정책 변경 시 단위 테스트 미갱신 (test_skip_when_* → test_fail_when_*) | MINOR |
| `[T10]` | Test determinism | unit test의 time.sleep mock 누락 (CI 시간 budget 직격) | MAJOR |
| `[T11]` | Test determinism | mock이 framework 동작과 불일치 (Fabric warn=False 기본값, UnexpectedExit raise 등) | MAJOR |
