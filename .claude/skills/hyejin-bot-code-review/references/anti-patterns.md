...
> **카탈로그 출처**: base는 upstream(`upstream-code-review-reference`) — 대연님의 누적된 카탈로그를 그대로 이어받음. hyejin delta는 `[G50]`, `[T20]`–`[T23]`, `[D20]`–`[D24]` 9개 룰. ssw-bundle / Robot Framework / release backport 같은 한혜진의 자주 보는 영역을 보강.
