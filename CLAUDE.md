...
| 9 | Daily news clip (feature 003) — generic `cron` trigger + `news` handler. The `cron` trigger is a wall-clock daily scheduler (no external API): it emits `news.daily` once per local-tz calendar day at/after a configured `HH:MM`, with `cron_state` tracking the last-fired date for fire-once-per-day across restarts. The `news` handler fetches Hacker News top stories (Firebase API, score-threshold fallback 200→150→100) + GeekNews RSS via `infra/news_sources.py`, summarizes the HN half with Claude (Korean headline + English bullets; GeekNews is title-only), renders a `≤4000`-char Slack message (split if longer), and DMs the operator via the shared Slack side-channel. Lands behind `[triggers.cron].enabled = false` + `[handlers.news].enabled = false`. Migration 007 adds `cron_state`. Manual fire: `hyejin-bot dev fire-news`. |

Built-in triggers: `manual`, `gh_review_requested`, `jira_assigned`, `cron`. Built-in handlers:
`echo`, `pr_review`, `jira_triage`, `news`. Other workloads (webhook, slack, digest, …) are
