...
from hyejin_bot.app.lifecycle import AlreadyRunningError, BootOptions, boot
...
def run(
    ...
    try:
        ...
        typer.echo(f"hyejin-bot: {exc}", err=True)
        ...
        typer.echo(f"hyejin-bot: {type(exc).__name__}: {exc}", err=True)
