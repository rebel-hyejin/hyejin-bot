...
outbox by `hyejin-bot dev fire …` (and the per-handler convenience
...
class ManualTrigger:
    ...
    Manual events are written to the outbox synchronously by `hyejin-bot dev fire`
    (see `hyejin_bot.cli.dev`), so there's nothing to do here. We keep the class
